...
library(outliers)
#removing null values and outliers with na.omit and rm.outlier
cleaned_realtordata1 <- na.omit(realtor_data1) 
cleaned_realtordata2 <- sapply(cleaned_realtordata1, rm.outlier, fill = TRUE) 

cleaned_realtordata <- cleaned_realtordata2 %>% data.frame() 
...
acre_lot =(acre_lot2-min(cleaned_realtordata$acre_lot))/(max(cleaned_realtordata$acre_lot))

house_size = (house_size2-min(cleaned_realtordata$house_size))/(max(cleaned_realtordata$house_size))

price10 = (price8-min(cleaned_realtordata$price))/(max(cleaned_realtordata$price))
...
```{r}
nn <- neuralnet(price~bed+bath+acre_lot+house_size,data=train_data,hidden=1, threshold=0.00001,linear.output=TRUE)
