...
predict_lm <- function(nor_User_data, linear_model_fit1 = linear_model_fit2) {
  if (class(linear_model_fit) != "lm"
  ) stop("This function needs an LM model")
  predict(linear_model_fit2, nor_User_data)
}
```
...
```{r}
neuralmodel <- neuralnet(price~bed+bath+house_size,data=train_data,hidden=2, threshold=0.01,act.fct=relu, rep=1, linear.output=TRUE)
...
```{r}
save(neuralmodel, file ="./neuralmodel3.RData")
```

```{r}
load("C:/Users/MAURINE/Desktop/STA 518/RealHomePrice/neuralmodel3.RData")
```
...
ANALYSIS OF TEST DATA PREDICTED PRICE VS ACTUAL PRICE

```{r}
test_data2 <- test_data %>% 
  select(-c(price))

test_data3 = test_data2[1:10000, ]
 

test_data3price <- PriceModel1(test_data3)

final_test = as.data.frame(test_data3price)
final_test
```
...
```{r}
test_dataprice<- test_data %>%
  select(price)

#test_dataprice

test_dataprice1 = head(test_dataprice,10000)
test_dataprice1[1]
```
...
```{r}
everything <-cbind(final_test, test_dataprice1)
#everything


names(everything)[1] = "predictedprice"
  
everything
```

```{r}
everything |> ggplot(aes(x =price, y = predictedprice))+ geom_point() + 
...
```{r}
save(everything, file= "tests2.RData")
```
