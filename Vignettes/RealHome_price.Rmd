...
```{r} 
realtor_data <- load("RealHomePrice/Data/cleaned_realtordata.rdata")
```
...
PriceModel <- neuralnet(price~bed+bath+house_size,data=test_data,hidden=1, threshold=0.00001,act.fct=relu, rep=1, linear.output=TRUE)
...
PriceModel1 <- function(nor_User_data) {
  load("Price_Model.Rdata")
  compute(Price_Model.Rdata, nor_User_data)
}
```
...
predict_Price <- function(user_data) {
  ...
  prices <- list("lm_price" = lm_price, "nn_price" = nn_price)
  return(prices)
